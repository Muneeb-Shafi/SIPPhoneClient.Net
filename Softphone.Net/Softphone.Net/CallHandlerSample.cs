...
namespace ozeki.voip.sip.client
{
    ...
    class CallHandlerSample
    {
        ...
        public CallHandlerSample(string registerName, string domainHost, string password)
        {
            ...
            InitializeSoftPhone(registerName, domainHost, password);
        }
        ...
        /// <param name="registerName">The SIP ID what will registered into your PBX</param>
        /// <param name="domainHost">The address of your PBX</param>
        private void InitializeSoftPhone(string registerName, string domainHost, string password)
        {
            try
            {
                softPhone = SoftPhoneFactory.CreateSoftPhone(5700, 5800);
                softPhone.IncomingCall += softPhone_IncomingCall;
                phoneLine = softPhone.CreatePhoneLine(new SIPAccount(true, registerName, registerName, registerName, password, domainHost));
        ...
        public RegState Call(string dialedNumber)
        {
            if(phoneLineInformation == RegState.RegistrationRequested) return RegState.RegistrationRequested;
            if (phoneLineInformation != RegState.RegistrationSucceeded)
            {
                Console.WriteLine("Phone line state is not valid!");
                return RegState.Error;
            }
            ...
            return RegState.RegistrationSucceeded;
        }
